...
function module.FetchHullsForPoint(self: CollisionModule, point: Vector3)
    local cell = self:FetchCell(point.X // self.gridSize, point.Y // self.gridSize, point.Z // self.gridSize)
    ...
    cell = self:FetchFatCell(point.X // self.fatGridSize, point.Y // self.fatGridSize, point.Z // self.fatGridSize)
...
function module.FetchHullsForBox(self: CollisionModule, min: Vector3, max: Vector3): { PartRecord }
    ...
    local key: Vector3 = Vector3.new(minx, minz, miny) --[[/]]
        / self.gridSize
    local otherKey: Vector3 = Vector3.new(maxx, maxy, maxz) --[[/]]
        / self.gridSize
    ...
    for record in hullMap do
        if record.hull == nil and record.instance then
            local hull = self:GenerateConvexHullAccurate(
                record.instance,
                module.expansionSize,
                self:GenerateSnappedCFrame(record.instance)
            )
...
function module.GenerateConvexHullAccurate(
    self: CollisionModule,
    part: BasePart,
    ...
    local hull, counter =
        MinkowskiSumInstance:GetPlanesForInstance(part, expansionSize, cframe, self.planeNum, debugRoot)
...
function module.PlaneLineIntersect(
    self: CollisionModule,
    normal: Vector3,
    distance: number,
    V1: Vector3,
