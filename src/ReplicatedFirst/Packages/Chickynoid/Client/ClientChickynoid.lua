...
function ClientChickynoid.HandleNewPlayerState(
    self: Class,
    stateDelta: LazyTable,
    ...
    if self.debug.useSkipResimulationOptimization == true then
        if lastConfirmed ~= nil then
            ...
            if cacheRecord then
                ...
                if
                    (cacheRecord.stateRecord.state.pos - stateRecord.state.pos).magnitude < 0.05
                    and (cacheRecord.stateRecord.state.vel - stateRecord.state.vel).magnitude < 0.1
                then
                    resimulate = false
                    -- print("skipped resim")
                end
