...
function ServerModule.WriteCharacterDataForSnapshots(): ()
    for userId, playerRecord in next, ServerModule.playerRecords do
        ...
        playerRecord.chickynoid.prevCharacterData[ServerModule.serverTotalFrames] =
            DeltaTable:DeepCopy(playerRecord.chickynoid.simulation.characterData)
...
function ServerModule.UpdatePlayerStatesToPlayers(): ()
    for userId, playerRecord in next, ServerModule.playerRecords do
        ...
        if playerRecord.chickynoid ~= nil then
            ...
            event.playerStateDelta, event.playerStateDeltaFrame =
                playerRecord.chickynoid:ConstructPlayerStateDelta(ServerModule.serverTotalFrames)
...
function ServerModule.WriteSnapshotsForPlayers(): ()
    ServerSnapshotGen:DoWork(
        ServerModule.playerRecords,
        ServerModule.serverTotalFrames,
        ServerModule.serverSimulationTime,
        ServerModule.flags.DEBUG_BOT_BANDWIDTH
    )
