...
function ServerChickynoid.HandleClientUnreliableEvent(self: ServerChickynoid, config: IServerConfig, event: {BinaryTable}, fakeCommand: boolean?): ()
	if (event[2] ~= nil) then
...
function ServerChickynoid.ProcessCommand(self: ServerChickynoid, config: IServerConfig, command: Command, fakeCommand: boolean?, resent: boolean?): ()
	if command and type(command) == "table" then
		if (command.localFrame == nil or type(command.localFrame) ~= "number" or command.localFrame ~= command.localFrame) then
			return
		end
...
function ServerChickynoid.ConstructPlayerStateDelta(self: ServerChickynoid, serverFrame: number): (SimStateRecord, number?)
